...
def parse_date_time(datetime_text):
    ...
    # Pattern: "Weekday • Month Day • Time-Time PM/AM"
    # Example: "Sat • Dec 6 • 6:00-8:00 PM"
    match = re.search(
        r'(\w{3})\s*•\s*(\w{3})\s+(\d{1,2})\s*•\s*(\d{1,2}:\d{2})-(\d{1,2}:\d{2})\s*(AM|PM)',
        datetime_text,
        re.IGNORECASE
    )
    ...
    if match:
        ...
        time_str = f"{start_time} - {end_time} {meridiem}"
